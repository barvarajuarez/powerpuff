...
// ==============================================================
// VERTICES: x, y, z,   nx, ny, nz,   u, v  (8 floats por vertice)
// ==============================================================
float vertices[] = {
    // BACK
...
// ==============================================================
// IDs DE TEXTURAS (una por cada color/material)
// ==============================================================
...
int main()
{
    ...
    // ----------------------------------------------------------
    // VAO / VBO  (stride = 8 floats: pos + normal + uv)
    // ----------------------------------------------------------
    GLuint VBO, VAO;
    ...
    // Atributo 2: UV (2 floats, offset 6)
    ...
    // ----------------------------------------------------------
    // CARGAR TEXTURAS
    // Pon tus imagenes PNG en la carpeta textures/
    // Son imagenes de color solido, puedes hacerlas en Paint de 64x64
    // ----------------------------------------------------------
    texMorado = cargarTextura("textures/morado.png");       // Base cama, respaldo, repisa, baul exterior, buros
    texGris = cargarTextura("textures/gris.png");         // Cajones de buros
    texVerde = cargarTextura("textures/verde.png");        // Franja izquierda cobija, base cama
    texRosa = cargarTextura("textures/rosa.png");         // Franja central cobija
    texAzul = cargarTextura("textures/azul.png");         // Franja derecha cobija
    texFucsia = cargarTextura("textures/fucsia.png");       // Respaldo cama (arco)
    texCrema = cargarTextura("textures/crema.png");        // Interior baul
    ...
    // Decirle al lightingShader que la textura diffuse esta en unidad 0
    lightingShader.Use();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Luz direccional (luz ambiente del cuarto)
        glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.direction"), -0.2f, -1.0f, -0.3f);
        glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.ambient"), 0.5f, 0.5f, 0.5f);
        glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.diffuse"), 0.4f, 0.4f, 0.4f);
        glUniform3f(glGetUniformLocation(lightingShader.Program, "dirLight.specular"), 0.0f, 0.0f, 0.0f);

        // Point light 1 (toggleable con SPACE)
        glm::vec3 lightColor;
        ...
        // ------------------------------------------------------
        // MODELOS .OBJ (piso y pared - tienen su propia textura)
        // ------------------------------------------------------
        model = glm::mat4(1);
        ...
        // --- RESPALDO (ARCO FUCSIA) ---
        float alturas[] = { 0.8f, 1.4f, 1.9f, 1.9f, 1.4f, 0.8f };
        float desplazamientoX = -2.0f;

        glBindTexture(GL_TEXTURE_2D, texFucsia); // morado oscuro
        for (int i = 0; i < 6; i++) {
            model = glm::mat4(1.0f);
            model = glm::translate(model, glm::vec3(desplazamientoX + (i * 0.8f), alturas[i] / 2.0f + 0.5f, -4.5f));
            model = glm::scale(model, glm::vec3(0.81f, alturas[i], 0.15f));
            glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
            glDrawArrays(GL_TRIANGLES, 0, 36);
        }

        // ALMOHADAS (gris claro, encima de la base cerca del respaldo)
        ...
        // --- CORAZON DECORATIVO DEL RESPALDO (ROSA) ---
        glBindTexture(GL_TEXTURE_2D, texRosa);
        model = glm::mat4(1);
        model = glm::translate(model, glm::vec3(0.0f, 1.1f, 4.48f));
        model = glm::rotate(model, glm::radians(45.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        model = glm::scale(model, glm::vec3(1.2f, 1.2f, 0.05f));
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glDrawArrays(GL_TRIANGLES, 0, 36);

        // --- BASE DE LA CAMA (FUCSIA) ---
        ...
        // --- COBIJA: FRANJA IZQUIERDA (VERDE) ---
        // Franja izquierda VERDE
        glBindTexture(GL_TEXTURE_2D, texVerde);
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(-1.2f, 0.55f, -2.0f));
        model = glm::scale(model, glm::vec3(1.4f, 0.12f, 5.0f)); // más larga en Z
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glDrawArrays(GL_TRIANGLES, 0, 36);

        // --- COBIJA: FRANJA CENTRAL (ROSA) ---
        glBindTexture(GL_TEXTURE_2D, texRosa);
        model = glm::mat4(1.0f);
        ...
        // ==========================================================
        // === BUROS (x2, izquierdo y derecho) ===
        // ==========================================================
        float posicionesX[] = { -2.8f, 2.8f };
        for (int i = 0; i < 2; i++) {
            // Estructura principal (morada)
            ...
            glBindTexture(GL_TEXTURE_2D, texGris);
            model = glm::mat4(1);
            model = glm::translate(model, glm::vec3(posicionesX[i], 0.7f, -3.69f));
            model = glm::scale(model, glm::vec3(0.6f, 0.3f, 0.05f));
            glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
            glDrawArrays(GL_TRIANGLES, 0, 36);

            // Cajon inferior (gris)
            model = glm::mat4(1);
            model = glm::translate(model, glm::vec3(posicionesX[i], 0.3f, -3.69f));
            model = glm::scale(model, glm::vec3(0.6f, 0.3f, 0.05f));
            glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
            glDrawArrays(GL_TRIANGLES, 0, 36);
        }
        ...
        // Revestimiento interior de la tapa (crema)
        glBindTexture(GL_TEXTURE_2D, texCrema);
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(4.3f, 0.4f, -2.5f));
        model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        // ------------------------------------------------------
        // LAMPARA (lamp shader, sin textura)
        // ------------------------------------------------------
        lampShader.Use();
...
// ==============================================================
// FUNCION AUXILIAR: cargar textura con stb_image
// ==============================================================
GLuint cargarTextura(const char* path)
{
    GLuint texID;

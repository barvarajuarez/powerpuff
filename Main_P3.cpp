...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
	    ...
	    view = glm::translate(view, glm::vec3(0.0f,0.0f,-12.5f));
		//SIGNIFICA ROTACION EL DE ABAJO
		...
		//PROYECCION ORTOGONALview = glm::translate( view, glm::vec3( screenWidth / 2, screenHeight / 2,-900.0f ) ); // use with orthographic projection
		...
		model = glm ::mat4(1);
		model = glm::translate(model, glm::vec3(5.0f, 0.0f, 0.0f));//LO MOVIO 5 ESPACIOS A LA IZQUIERDA
		//OTRA VEZ
		model = glm::rotate(model, 0.5f, glm::vec3(1.0f, 1.0f, 0.0f)); // use to compare orthographic and perspective projection
		//ESCALA
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 9.0f));
		glUniformMatrix4fv(modelLoc,1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		glBindVertexArray(0);
